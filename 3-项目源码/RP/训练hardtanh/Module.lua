...
function Module:parameters()
   if self.weight and self.bias self.projection then
      return {self.weight, self.bias, self.projection}, {self.gradWeight, self.gradBias, self.gradProjection}
   elseif self.weight and self.bias then
      return {self.weight, self.bias}, {self.gradWeight, self.gradBias}
   elseif self.weight and self.projection then
      return {self.weight, self.projection}, {self.gradWeight, self.gradProjection}
   elseif self.bias and self.projection then
      return {self.bias, self.projection}, {self.gradBias, self.gradProjection}
   elseif self.weight then
      return {self.weight}, {self.gradWeight}
   elseif self.bias then
      return {self.bias}, {self.gradBias}
   elseif self.projection then
      return {self.projection}, {self.gradProjection}    
   else
      return
   end
end
