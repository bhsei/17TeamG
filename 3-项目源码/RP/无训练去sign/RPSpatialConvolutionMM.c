...
#define TH_GENERIC_FILE "generic/RPSpatialConvolutionMM.c"
#else

static inline void THNN_(RPSpatialConvolutionMM_shapeCheck)(
...
static void THNN_(RPSpatialConvolutionMM_updateOutput_frame)(
          THTensor *input,
          THTensor *output,
          THTensor *weight,
          THTensor *bias,
...
void THNN_(RPSpatialConvolutionMM_updateOutput)(
          THNNState *state,
          THTensor *input,
          THTensor *output,
          THTensor *weight,
          ...
          int padH)
{
  ...
  THNN_(RPSpatialConvolutionMM_shapeCheck)
  ...
  if(input->nDimension == 3)
  {
    ...
    THNN_(RPSpatialConvolutionMM_updateOutput_frame)
...
static void THNN_(RPSpatialConvolutionMM_updateGradInput_frame)(
          THTensor *gradInput,
          THTensor *gradOutput,
...
//
  THTensor *Biweight = THTensor_(newWithSize2d)(gradOutput->size[0], projection->size[1]);
  THTensor_(addmm)(Biweight, 0, Biweight, 1, weight, projection); 
  THTensor_(sign)(Biweight,Biweight);
  THTensor *tBiweight = THTensor_(new)();
  THTensor_(transpose)(tBiweight, Biweight, 0, 1);

  THTensor *delta = THTensor_(newWithSize2d)(projection->size[0], gradOutput->size[0]); //delta = projection * Biweight:t()
  THTensor_(addmm)(delta, 0, delta, 1, projection, tBiweight);  
  THTensor_(addmm)(fgradInput, 0, fgradInput, 1, delta, gradOutput2d);
//THTensor_(addmm)(fgradInput, 0, fgradInput, 1, projection * Biweight:t(), gradOutput2d);  

  THTensor_(free)(Biweight);
  THTensor_(free)(tBiweight);
  THTensor_(free)(delta);
//shen mingzhu 
...
void THNN_(RPSpatialConvolutionMM_updateGradInput)(
          THNNState *state,
          THTensor *input,
          THTensor *gradOutput,
          THTensor *gradInput,
...
//  the original put the tweight into the frame function as weight
  if(input->nDimension == 3)
  {
    ...
    THNN_(RPSpatialConvolutionMM_updateGradInput_frame)(gradInput, gradOutput,
						      weight, fgradInput, projection,
						      kW, kH, dW, dH, padW, padH);
  }
  else
  {
...
#pragma omp parallel for private(t)
    for(t = 0; t < T; t++)
    {
      ...
      THNN_(RPSpatialConvolutionMM_updateGradInput_frame)(gradInput_t, gradOutput_t,
							weight, fgradInput_t, projection,
							kW, kH, dW, dH, padW, padH);
...
static void THNN_(RPSpatialConvolutionMM_accGradParameters_frame)(
          THTensor *gradOutput,
          THTensor *gradWeight,
          THTensor *gradBias,
          THTensor *finput,
          ...
          real scale)
{
  ...
  /*THTensor *tprojection = THTensor_(new)();
  THTensor_(transpose)(tprojection, projection, 0, 1);
  THTensor *Pinput = THTensor_(newWithSize2d)(projection->size[1], gradOutput->size[1]*gradOutput->size[2]);
  THTensor_(addmm)(Pinput, 0, Pinput, 1, tprojection, finput);
  THTensor *Binput = THTensor_(new)();
  THTensor_(sign)(Binput,Pinput);*/
  THTensor *tprojection = THTensor_(new)();
  THTensor_(transpose)(tprojection, projection, 0, 1);
  THTensor *Binput = THTensor_(newWithSize2d)(projection->size[1],gradOutput->size[1]*gradOutput->size[2]);
  THTensor_(addmm)(Binput, 0, Binput, 1, tprojection, finput);
  THTensor_(sign)(Binput,Binput);

  THTensor *delta = THTensor_(newWithSize2d)(projection->size[0], gradOutput->size[1]*gradOutput->size[2]);
  THTensor_(addmm)(delta, 0, delta, 1, projection, Binput);
  THTensor *tdelta = THTensor_(new)();
  THTensor_(transpose)(tdelta, delta, 0, 1);
  THTensor_(addmm)(gradWeight, 1, gradWeight, scale, gradOutput2d, tdelta);

  THTensor_(free)(tprojection);   
  THTensor_(free)(Binput);
  THTensor_(free)(delta);
  THTensor_(free)(tdelta);
  //Shen Mingzhu
//  Original one 
...
void THNN_(RPSpatialConvolutionMM_accGradParameters)(
          THNNState *state,
          THTensor *input,
          THTensor *gradOutput,
          THTensor *gradWeight,
          ...
          real scale)
{
  ...
  {
    //
    THNN_(RPSpatialConvolutionMM_accGradParameters_frame)(gradOutput, gradWeight,
              gradBias, finput, projection, scale);
    //  
}
  else
  {
    ...
    for(t = 0; t < T; t++)
    {
      ...
      //
      THNN_(RPSpatialConvolutionMM_accGradParameters_frame)(gradOutput_t, gradWeight,
                gradBias, finput_t, projection, scale);
      //
